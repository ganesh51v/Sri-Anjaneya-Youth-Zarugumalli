...
const OtpInput = ({ value, onChange }) => {
  ...
  const inputClass =
    "w-10 h-12 text-center text-lg font-extrabold rounded-xl border-2 border-cream-300 focus:border-saffron-500 focus:ring-2 focus:ring-saffron-500/20 bg-cream-50/50 dark:bg-slate-950 dark:border-slate-700 dark:text-white outline-none transition-all font-mono";
  return (
    <div className="flex gap-2 justify-center" onPaste={handlePaste}>
      {[0, 1, 2, 3, 4, 5].map((i) => (
        <input
          ...
          }} // Support for Android IME which doesn't reliably trigger onKeyDown
...
const SignUp = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      ...
      const channelLabel =
        otpChannel === "phone" ? `📱 ${destination}` : `✉️ ${destination}`;
      setInfo(
        `OTP sent to ${channelLabel}. Enter the 6-digit code below to verify and create your account.`,
      );
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-cream-100/30 relative">
      <SEO
        title="Sign Up"
        description="Join Sri Anjaneya Youth Zarugumalli. Register as a new member and participate in seva, cultural events and community activities."
        path="/signup"
      />
      {/* Language Switcher */}
      ...
      <div className="flex-grow flex items-center justify-center p-4 sm:p-8">
        <div
          ref={cardRef}
          style={{ opacity: 0 }}
          className="w-full max-w-lg bg-white rounded-3xl shadow-xl border border-cream-200 overflow-hidden relative glass-panel"
        >
          {/* Header */}
          <div className="bg-gradient-to-br from-saffron-500 via-saffron-600 to-devored-700 text-white px-6 py-6 text-center relative overflow-hidden">
            ...
            <div className="relative z-10 flex justify-center gap-2 mb-3">
              {[1, 2].map((n) => (
                <div
                  key={n}
                  className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-extrabold transition-all ${
                    step >= n
                    ...
                    <div className="mt-1.5 pl-1">
                      <div className="flex gap-1 mb-0.5">
                        {[1, 2, 3, 4].map((n) => (
                          <div
                            key={n}
                            className={`h-1 flex-1 rounded-full transition-all ${n <= pwStrength.level ? pwStrength.color : "bg-cream-200"}`}
                          />
                        ))}
                      </div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-1.5 pl-1">
                    {otpChannel === "phone"
                      ? language === "en"
                        ...
                        : `SMS ద్వారా OTP +91${phone || "XXXXXXXXXX"} కు పంపబడుతుంది`
                      : language === "en"
                        ? `OTP will be sent to ${email || "your email address"}`
                        : `OTP ${email || "మీ ఈమెయిల్ చిరునామా"} కు పంపబడుతుంది`}
                  </p>
                </div>
                {/* Send OTP button */}
            ...
            {step === 1 && (
              <>
                <div className="relative flex py-1 items-center">
                  ...
                  <span className="flex-shrink mx-3 text-xs text-slate-400 uppercase tracking-widest font-semibold">
                    {t("or")}
                  </span>
                  <div className="flex-grow border-t border-cream-200" />
                </div>
