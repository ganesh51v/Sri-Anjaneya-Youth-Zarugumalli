...
const Profile = () => {
    ...
    try {
      ...
      if (photoFile) {
        try {
          ...
          console.warn(
            "[Profile] Cloudinary upload notice:",
            uploadErr.message,
          );
          setError(
            "Photo upload failed — profile saved without new photo. Please try again.",
          );
          finalPhotoUrl = user?.photoUrl || "";
        }
      }
      await dbService.users.updateDetails(userId, {
        name,
        phone,
        village,
  ...
  const handleRequestCommittee = async () => {
    ...
    try {
      ...
      setError(
        language === "en"
          ? "Failed to request committee membership."
          : "కమిటీ సభ్యత్వం అభ్యర్థించడంలో విఫలమైంది.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex-1 max-w-lg mx-auto px-4 py-10 w-full text-[var(--text-primary)]">
      <SEO
        title="My Profile"
        description="Manage your Sri Anjaneya Youth Zarugumalli member profile."
        path="/profile"
        noindex={true}
      />
      <div
        ref={cardRef}
        style={{ opacity: 0 }}
        className="bg-[var(--bg-card)] rounded-[28px] border-2 border-gold-500/30 shadow-2xl overflow-hidden relative transition-colors duration-300"
      >
        {/* Header Ribbon */}
        ...
        <div className="p-8 text-center border-b border-[var(--border)] bg-gradient-to-b from-saffron-500/10 via-[var(--bg-muted)]/50 to-transparent relative">
          <div
            ref={avatarRef}
            style={{ opacity: 0 }}
            className="w-24 h-24 rounded-full bg-gradient-to-tr from-saffron-500/20 to-gold-500/20 border-4 border-gold-400 shadow-xl mx-auto flex items-center justify-center text-saffron-600 dark:text-saffron-400 text-3xl font-black overflow-hidden relative group"
          >
            {(isEditing ? photoPreview : photoUrl) ? (
              <img
                src={isEditing ? photoPreview : photoUrl}
                alt={name || "User"}
        ...
        <div className="p-6 sm:p-8 space-y-5">
          ...
          {isChangingPassword ? (
            <form onSubmit={handlePasswordChange} className="space-y-4">
              <div>
                <label className="block text-[11px] font-black uppercase tracking-wider text-[var(--text-secondary)] mb-1.5 pl-1">
                  {t("newPassword")}
                </label>
                <input
                  type="password"
              ...
              <div className="grid grid-cols-1 gap-3 text-xs">
                ...
                <div className="flex items-center gap-3.5 bg-[var(--bg-muted)] border border-[var(--border)] p-4 rounded-2xl shadow-sm transition-all hover:border-gold-500/40">
                  ...
                  <div className="min-w-0 flex-1">
                    <span className="block text-[10px] font-black uppercase tracking-wider text-[var(--text-muted)] mb-0.5">
                      {t("emailAddress")}
                    </span>
                    <span className="font-extrabold text-sm text-[var(--text-primary)] truncate block">
                      {user?.email}
                    </span>
                  </div>
                </div>
                ...
                {user?.role === "admin" && (
                  <div className="flex items-center gap-3.5 bg-gold-500/10 border border-gold-500/30 p-4 rounded-2xl">
                    ...
                    <div>
                      <span className="block text-[10px] font-black uppercase tracking-wider text-gold-600 dark:text-gold-400">
                        {t("adminAccess")}
                      </span>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <span className="text-xs font-black text-[var(--text-primary)]">
                        ...
                        {(!user?.committeeStatus ||
                          user?.committeeStatus === "none") && (
                          <span className="text-[var(--text-secondary)] font-bold mt-0.5 block">
                            {t("notACommitteeMember")}
                          </span>
                        )}
                      </span>
                    </div>
