...
const Members = () => {
  ...
  const fetchMembers = async (isMounted) => {
    setLoading(true);
    setError("");
    try {
  ...
  useEffect(() => {
    if (headerRef.current)
      fadeUp(headerRef.current, { delay: 0, distance: 20 });
  }, []);

  useEffect(() => {
    if (!loading && membersGridRef.current) {
      staggerScaleFade(
        membersGridRef.current.querySelectorAll(":scope > div"),
        { stagger: 60, startDelay: 100 },
      );
    }
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        "Are you sure you want to remove this member from the database?",
      )
    ) {
      try {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingMember) {
        const updated = await dbService.members.update(
          editingMember.id,
          memberData,
        );
        setMembers((prev) =>
    ...
    <div className="flex-1 max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 space-y-6 xl:space-y-8 w-full">
      ...
      <div className="relative max-w-md xl:max-w-lg">
        ...
        <input
          ...
          className="w-full bg-white dark:bg-slate-900 border border-cream-300 dark:border-slate-800 rounded-xl py-2.5 xl:py-3 pl-11 pr-4 text-xs sm:text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-saffron-500 text-slate-800 dark:text-white transition-all"
        />
        {searchTerm && (
          <button
      ...
      {loading ? (
        ...
        <div
          ref={membersGridRef}
          className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 xl:gap-8"
        >
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 xl:space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  required
                >
                  <option value="President / Founder">
                    President / Founder
                  </option>
                  ...
                  <option value="Seva Representative">
                    Seva Representative
                  </option>
              ...
              <div>
                ...
                {/* Image Upload Box */}
                <div className="flex items-center gap-4 bg-cream-50 dark:bg-slate-800/40 border border-cream-300 dark:border-slate-700 rounded-xl p-3 mb-2">
                  <div className="w-12 h-12 rounded-full bg-saffron-100 dark:bg-saffron-950 border border-cream-300 dark:border-slate-700 flex items-center justify-center text-saffron-600 font-bold overflow-hidden shrink-0">
                    {photoUrl ? (
                      <img
                        src={photoUrl}
                        alt="Preview"
                  ...
                  <div className="flex-1">
                    ...
                    <label
                      ...
                      className="inline-block px-3 py-1.5 bg-white dark:bg-slate-800 border border-cream-300 dark:border-slate-700 rounded-lg text-xs font-bold text-slate-700 dark:text-slate-200 hover:bg-cream-50 cursor-pointer transition-colors"
                    >
                      {t("chooseImage")}
                    </label>
                ...
                <div className="relative flex py-2 items-center">
                  <div className="flex-grow border-t border-cream-200 dark:border-slate-800"></div>
                ...
                <div className="mt-2.5">
                  <span className="block text-[10px] xl:text-xs font-semibold text-slate-400 uppercase mb-1.5">
                  ...
                  <div className="flex gap-2.5">
                    {avatarPresets.map((preset, idx) => (
                      <button
                        ...
                        className={`w-8 h-8 rounded-full overflow-hidden border-2 cursor-pointer transition-all ${photoUrl === preset ? "border-saffron-500 scale-110" : "border-transparent hover:border-cream-300"}`}
                      >
                        <img
                          src={preset}
                          className="w-full h-full object-cover"
