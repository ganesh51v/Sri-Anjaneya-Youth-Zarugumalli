...
const Gallery = () => {
  ...
  const fetchAlbums = async (isMounted) => {
    setLoading(true);
    setError('');
    try {
      ...
      if (isMounted) setError(t('failedFetchGallery') || 'Failed to fetch gallery albums.');
      console.error(err);
    } finally {
  ...
  return (
    <div className="flex-1 max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 space-y-6 xl:space-y-8 w-full">
      ...
      {/* Header Panel */}
      <div className="border-b border-cream-250 dark:border-slate-800 pb-5 xl:pb-6">
        <h1 className="text-2xl sm:text-3xl xl:text-4xl font-black text-slate-800 dark:text-white tracking-tight">
      ...
      {loading ? (
        <div className="py-20 flex flex-col items-center justify-center">
          ...
          <p className="mt-2 text-xs sm:text-sm text-slate-400">{t('loadingGallery')}</p>
        </div>
      ) : albums.length > 0 ? (
        <div ref={albumsGridRef} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 xl:gap-8">
          {albums.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className="overflow-hidden aspect-video relative bg-slate-950">
                  ...
                  />
                  {/* Photos count badge */}
                  <span className="absolute bottom-3 left-3 bg-black/60 backdrop-blur-sm text-white px-2.5 py-1 rounded-md text-xs font-bold tracking-widest font-mono">
                    {album.imageUrls.length} {t('photosCount').toUpperCase()}
                  </span>
                  {/* Date Badge */}
                  {album.eventDate && (
                    <span className="absolute top-3 left-3 bg-slate-900/70 text-gold-300 border border-gold-500/20 px-2.5 py-1 rounded-full text-xs font-bold flex items-center gap-1.5">
                ...
                {/* Card Body */}
                <div className="p-5 xl:p-6 flex-1 flex flex-col justify-between bg-white dark:bg-slate-900">
                  <div className="space-y-2">
                    <h3 className="text-base xl:text-lg font-black text-slate-800 dark:text-white line-clamp-1">
                      {album.title}
                    </h3>
                  ...
                  <span className="text-xs xl:text-sm font-bold text-saffron-600 dark:text-saffron-400 flex items-center gap-1 mt-4">
      ...
      {selectedAlbum && (
        <div className="fixed inset-0 z-40 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4 md:p-8 overflow-y-auto animate-fade-in">
          <div ref={modalPanelRef} style={{ opacity: 0 }} className="bg-white dark:bg-slate-900 border border-cream-200 dark:border-slate-800 rounded-3xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-saffron-500 to-saffron-600 text-white px-6 py-5 xl:px-8 xl:py-6 flex justify-between items-start gap-4">
              <div>
                <h2 className="font-extrabold text-base xl:text-lg uppercase tracking-wider">
                  {selectedAlbum.title}
                </h2>
                <p className="text-xs sm:text-sm text-saffron-100 mt-1 line-clamp-2">
                  {selectedAlbum.description}
                </p>
                {selectedAlbum.eventDate && (
                  <span className="text-xs text-gold-300 font-bold block mt-1">
              ...
              <button 
                ...
                className="text-white hover:text-saffron-100 bg-white/10 hover:bg-white/20 p-2 rounded-full cursor-pointer transition-colors shrink-0"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto p-6 md:p-8 xl:p-10 flex-1 bg-cream-50/10 dark:bg-slate-950/20">
...
const Lightbox = ({ album, index, setIndex, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/95 flex flex-col justify-between select-none animate-fade-in">
      {/* Lightbox Header */}
      <div className="p-4 sm:p-6 flex items-center justify-between bg-gradient-to-b from-black/60 to-transparent z-10">
        <span className="text-white font-mono text-xs sm:text-sm font-bold tracking-widest bg-black/40 px-3.5 py-1.5 rounded-full">
          {index + 1} / {album.imageUrls.length}
        </span>
        <div className="flex items-center gap-3 sm:gap-4">
          <button
            onClick={handleDownload}
            className="text-white hover:text-saffron-400 bg-black/40 p-2.5 rounded-full cursor-pointer hover:scale-105 transition-all"
      ...
      >
        ...
        {album.imageUrls.length > 1 && (
          <>
            <button
              onClick={handlePrev}
              className="absolute left-4 md:left-8 bg-black/40 hover:bg-black/60 text-white p-3.5 rounded-full cursor-pointer hover:scale-105 transition-all z-10 hidden sm:flex items-center justify-center"
            >
              <ChevronLeft className="w-6 h-6 sm:w-8 sm:h-8" />
            </button>
            <button
              onClick={handleNext}
              className="absolute right-4 md:right-8 bg-black/40 hover:bg-black/60 text-white p-3.5 rounded-full cursor-pointer hover:scale-105 transition-all z-10 hidden sm:flex items-center justify-center"
            >
      ...
      {/* Lightbox Footer */}
      <div className="p-5 sm:p-6 text-center bg-gradient-to-t from-black/80 to-transparent z-10 text-white space-y-1">
        <h4 className="font-extrabold text-sm sm:text-base tracking-wide uppercase">{album.title}</h4>
        {album.description && (
    ...
    <div className="relative max-w-full max-h-full flex items-center justify-center p-4">
      {loading && !error && (
        <div className="absolute inset-0 flex items-center justify-center">
          <Loader2 className="w-8 h-8 sm:w-10 sm:h-10 text-saffron-500 animate-spin" />
        </div>
      )}
      {error ? (
        <div className="flex flex-col items-center justify-center text-slate-500 gap-2 p-10 border border-dashed border-slate-800 rounded-2xl bg-slate-900/40">
          ...
          <span className="text-xs sm:text-sm font-semibold">{t ? t('imageFailedToLoad') : 'Image failed to load'}</span>
        </div>
      ) : (
        <img
          src={src}
