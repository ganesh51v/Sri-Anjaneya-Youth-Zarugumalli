      ...
      <div className="p-5 flex items-start gap-4 flex-1">
        ...
        <div className="relative shrink-0">
          {member.photoUrl ? (
            <img
              src={member.photoUrl}
              alt={`${translateName(member.name, language)} - ${translateRole(member.role, language) || 'Member'}, Sri Anjaneya Youth Zarugumalli`}
              width="56"
              height="56"
              loading="lazy"
            ...
            <div
              className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${getGradient(
                member.name,
              )} flex items-center justify-center text-white font-black text-xl group-hover:scale-105 transition-all duration-300 shadow-md`}
            >
          ...
          {isOfficer && (
            <div
              className="absolute -bottom-1.5 -right-1.5 bg-gold-500 text-white p-1 rounded-lg border-2 border-[var(--bg-card)] shadow-sm"
              title={t('officeBearer')}
            >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] font-bold text-saffron-600 mb-2.5 uppercase tracking-wide">
            {translateRole(member.role, language)}
          </p>
          <div className="space-y-1.5">
            <a
              href={`tel:${member.phone}`}
              className="flex items-center gap-1.5 text-xs text-[var(--text-muted)] hover:text-saffron-500 transition-colors group/link"
            >
            ...
            <div className="flex items-center gap-1.5 text-xs text-[var(--text-muted)]">
              ...
              <span className="truncate">{translateAddress(member.area, language)}</span>
            </div>
          </div>
        </div>
      </div>
